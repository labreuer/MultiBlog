-- /keywords' derived columns, as a view (PLAN.md §20d, §16e, §16l). Its own
-- migration rather than an append to add_keywords, matching add_doc_metrics_view:
...
--
-- WHY THIS GROUPS keyword_assignment/keyword_anchor RATHER THAN SELECTING FROM
-- keyword. A view that reads its own base table gets that table scanned twice on
...
-- owned tables removes the second reference to `keyword` instead of hoping it is
...
-- doc_metrics carries: a keyword nobody has used has **no row here at all**,
-- where a `FROM keyword` form would give it a row of zeroes. The page reads
-- `keyword.metrics?.x ?? 0` and orders the relation `nulls: "last"`, so an
...
-- permission-filtered counts a *reader* sees come from /keyword/[slug]'s own
...
CREATE VIEW keyword_metrics AS
WITH live_anchor AS (
    SELECT a.keyword_id,
    ...
    FROM keyword_assignment a
    JOIN keyword_anchor k ON k.assignment_id = a.id
    -- Exactly one of these four joins matches, by keyword_anchor_one_target_check
