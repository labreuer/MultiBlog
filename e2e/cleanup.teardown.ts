...
teardown("remove throwaway users, posts, docs and commenters", async () => {
  const { posts, docs, users, ydocs } = await sweepTestData();
  console.log(`e2e cleanup: removed ${posts} post(s), ${docs} doc(s), ${users} user(s), ${ydocs} ydoc(s).`);
  disconnect();
});
