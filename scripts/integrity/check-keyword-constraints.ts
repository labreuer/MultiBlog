// Proves the hand-written DDL in add_keywords actually enforces what its
...
//     well-typed. A `keyword_anchor` with two FKs set is a perfectly ordinary
//     Prisma create call.
...
// rows are real ones (a real keyword, a real assignment, against a real user
// and doc), which is why it needs some content to exist at all.
//
// Usage:
//   npx tsx scripts/integrity/check-keyword-constraints.ts
...
type Fixtures = {
  ...
  docId: string;
  postId: string;
  /** The probe keyword's exact name, so the case-collision probe can differ from it *only* by case. */
  keywordName: string;
};
...
const MUST_REJECT: Probe[] = [
  {
    ...
    constraint: "keyword_anchor_one_target_check",
    why: "an anchor that names nothing is not a degraded anchor — it is a row with no meaning (§20b)",
    attempt: (tx, f) =>
      tx.$executeRaw`INSERT INTO keyword_anchor (id, assignment_id) VALUES ('probe-none', ${f.assignmentId})`,
  },
  {
  ...
  },
  {
    ...
    attempt: (tx, f) =>
      tx.$executeRaw`
        INSERT INTO keyword (id, slug, name, created_by_id, created_at)
        VALUES ('probe-case', 'probe-case-slug', ${f.keywordName.toUpperCase()}, ${f.userId}, now())`,
  },
];
...
const MUST_ACCEPT: Probe[] = [
  {
    ...
    attempt: (tx, f) =>
      tx.$executeRaw`
        INSERT INTO keyword_anchor (id, assignment_id, doc_id)
  ...
  },
  {
    ...
    attempt: (tx, f) =>
      tx.$executeRaw`
        INSERT INTO keyword_anchor (id, assignment_id, doc_id, selector_kind, anchor_from, anchor_to, quoted_text)
...
async function main() {
  ...
  await prisma
    .$transaction(async (tx) => {
      const keywordName = `Probe Constraint Term ${Date.now()}`;
      const keyword = await tx.keyword.create({
        data: { slug: `probe-${Date.now()}`, name: keywordName, createdById: user.id },
        select: { id: true },
      });
      const assignment = await tx.keywordAssignment.create({
        data: { keywordId: keyword.id, userId: user.id },
        select: { id: true },
      });
