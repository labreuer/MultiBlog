...
## `check-keyword-constraints.ts` — the odd one out, again

Every other script here verifies **stored data**. This one verifies the **schema**: that
`add_keywords`' two hand-written CHECK constraints and its `lower(name)` unique index actually
...
`check-annotation-anchors.ts` also walks `keyword_anchor` since PLAN.md §20g: the replay
invariant is a per-row property, so one checker covers every anchor table rather than one per
consumer family. In PR 1 that walk reports `0 of 0` — every keyword anchor is whole-object and
