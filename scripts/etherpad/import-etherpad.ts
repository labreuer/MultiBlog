...
//
// A DOC'S ID IS AN ORDINARY CUID, AND THE SLUG IS THE ONLY IMPORT KEY
// An imported doc gets its id from the schema's own @default(cuid()), exactly
// like one created through the app. It used to be a derived
// `etherpad-<slug-of-pad-name>`, which made the id a redundant second copy of
// the slug and stamped "imported" onto every admin URL and every ydoc:<id>
// document name for good. The slug itself never carried the prefix and still
// doesn't: it is claimed from the title through claimSlug, the same as any
// other doc's.
//
// What that costs is re-run recognition. There is no longer anything
// deterministic tying a pad to the row a previous run made, so --replace
// matches on the slug the pad wants — it deletes whatever doc holds that slug
// — and without --replace a taken slug is a hard error rather than a quiet
// second copy at -2. A doc that has nothing to do with Etherpad but sits on the
// slug will therefore stop the import; deciding what to do about that is a
// human's call, not a suffix's.
//
  ...
  try {
    await prismaIncludingDeleted.$transaction(
      async (tx) => {
        ...
        // Within a group of pads whose titles slugify alike, the one with the
        // most revisions goes first so it claims the unsuffixed url. Plain
        // alphabetical order would hand /doc/eve-and-the-serpent to the empty
        // pad created by a mistyped title and push the real document to -2,
        // because a space sorts before an underscore. Sorted on the slug of the
        // *title* rather than of the raw padId, because the title is what the
        // slug is actually derived from — prettyTitle collapses underscores to
        // spaces, so two pads can share a title slug without sharing a padId one.
        const importOrder = [...padIds].sort((a, b) => {
          const sa = slugify(prettyTitle(a, titleMode), "doc");
          const sb = slugify(prettyTitle(b, titleMode), "doc");
        ...
        // The slug each pad wants, assigned across importOrder as if the
        // database were empty. Two pads whose titles slugify alike — "Eve and
        // the serpent " and "Eve_and_the_serpent", since Etherpad has no rename
        // and a retitle means a second pad — resolve here, deterministically,
        // rather than falling out of whatever claimSlug happened to find taken.
        //
        // That determinism is exactly what makes --replace idempotent. Matching
        // a pad against the database on its *unsuffixed* base slug would find
        // only the first pad of each such group; the second's -2 row would never
        // be recognised, and re-running would strand it and create a -3. Here
        // the second pad asks for -2 by construction and matches its own row.
        const desiredSlugs = new Map<string, string>();
        const slugRun = new Map<string, number>();
        for (const padId of importOrder) {
          const base = slugify(prettyTitle(padId, titleMode), "doc");
          const n = (slugRun.get(base) ?? 0) + 1;
          slugRun.set(base, n);
          desiredSlugs.set(padId, n === 1 ? base : `${base}-${n}`);
        }
        ...
        for (const padId of importOrder) {
          ...
          // A doc's id is an ordinary cuid from the schema's own @default, the
          // same as a doc created through the app. It used to be a derived
          // `etherpad-<slug>`, which made the id a second, redundant copy of
          // the slug and marked every imported doc as second-class forever —
          // visible in every /doc admin URL and every ydoc:<id> document name.
          //
          // The cost is that an imported doc is no longer recognisable as
          // "this pad's row" by id, so --replace has nothing deterministic to
          // match on. The slug is the only stable, human-meaningful key left,
          // and it is what a re-import would collide on anyway, so that is what
          // it matches: --replace deletes whatever doc currently holds the slug
          // this pad wants, and without it a taken slug is an error rather than
          // a silent second copy at -2. An unrelated doc already sitting on the
          // slug therefore blocks the import instead of quietly displacing it,
          // which is the right way round — that is a decision for a human.
          //
          const desiredSlug = desiredSlugs.get(padId)!;
          if (desiredSlug !== slugify(title, "doc")) {
            summary.renamedSlugs.push(`${padId} → /doc/${desiredSlug} (title collides with another pad)`);
          }

          const clash = await tx.doc.findFirst({ where: { slug: desiredSlug }, select: { id: true } });
          if (clash) {
            if (!replace) {
              throw new Error(
                `A doc already exists at /doc/${desiredSlug} (${clash.id}), which pad ` +
                  `${JSON.stringify(padId)} wants. Pass --replace to delete and redo it.`,
              );
            }
            ...
            await tx.ydoc.deleteMany({ where: { id: ydocIdForDoc(clash.id) } });
            await tx.doc.delete({ where: { id: clash.id } });
            summary.replaced.push(`${padId} (${clash.id} at /doc/${desiredSlug})`);
          }
          ...
          // Asks for the slug resolved above, not for the title, so an in-run
          // collision is already settled and only a doc outside this import can
          // push it further.
          const slug = await claimSlug(tx, "doc", desiredSlug, claimedDocSlugs);
          if (slug !== desiredSlug) {
            summary.renamedSlugs.push(`${padId} → /doc/${slug} (/doc/${desiredSlug} was taken)`);
          }
          ...
          // No `id`: the schema's @default(cuid()) supplies it, so an imported
          // doc is indistinguishable from one the app created. Everything below
          // keys off the id that comes back, since nothing can predict it.
          const { id: docId } = await tx.doc.create({
            data: {
              slug,
              title,
              visibility,
