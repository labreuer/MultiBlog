...
// A function of the phase rather than a plain object, for reactMaxHeadersLength
// at the bottom — the one setting here that has to differ between `next dev`
...
export default function nextConfig(phase: string): NextConfig {
  return {
    ...
    // React's streaming renderer emits a `Link: <…>; rel=preload; as="style"`
    // response header for every stylesheet, and Next forwards it — in dev too.
    // Safari treats a *preloaded* resource as reusable without validation,
    // `Cache-Control: no-cache` and a changed ETag notwithstanding (WebKit bug
    // 193533, open since 2019), and dev chunk URLs don't change with their
    // content — so a phone on the LAN kept the old stylesheet through every
    // Reload after a CSS edit, rendering new elements with no styling at all,
    // until a private window came up with an empty cache. The value is React's
    // header budget in bytes and the only switch for that header; 1 rather
    // than 0 because React console.errors on every render for zero or less,
    // and no preload entry can fit in one byte, so the effect is the same
    // minus the noise. Production keeps the default (6000): there the chunk
    // URLs are content-hashed, and the preload is a real win. docs/ENV.md,
    // "Testing on a real phone".
    ...(phase === PHASE_DEVELOPMENT_SERVER ? { reactMaxHeadersLength: 1 } : {}),
  };
}
