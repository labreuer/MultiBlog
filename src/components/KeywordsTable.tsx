...
import { type KeywordsFilters, buildKeywordsQueryString } from "@/lib/keywords-query";
...
// PLAN.md §20d — /keywords, built from the shared table kit exactly as /files
...
export default function KeywordsTable({
  ...
  const { navigate, updateFilters, searchDraft, onSearchChange, handleSort, searchParams } = useTableFilters({
    filters,
    build: (next, extra) => buildKeywordsQueryString(next, extra, prefs),
  });
  ...
  // axis but existence.
  const bulkActions: BulkAction<KeywordRow>[] = canCurate
    ? softDeleteBulkActions<KeywordRow>("keywords", bulkDeleteKeywords, bulkRestoreKeywords)
    : [];
  ...
  const columns: ColumnSpec<KeywordRow>[] = [
    {
      key: "select",
      alwaysVisible: true,
      header: "Select",
    ...
    },
    {
      ...
      header: "Name",
      sortKey: "name",
      // Links to the public browse page, not to an edit form: /keyword/[slug]
      ...
      cell: (row) => (
        <span className={styles.nameCell}>
          <Link href={`/keyword/${row.slug}`}>{row.name}</Link>
          {canCurate && !row.deleted && (
            <button
              ...
              aria-label={`Rename keyword ${row.name}`}
            >
              ✎
            </button>
          )}
  ...
  function handleDeleteToggle(row: KeywordRow) {
    setError(null);
    startTransition(async () => {
      try {
        await runWithStatus(row.id, async () => {
          if (row.deleted) {
            await restoreKeyword(row.id);
          } else {
  ...
  function handleRename(row: KeywordRow) {
    ...
    startTransition(async () => {
      try {
        await runWithStatus(row.id, () => renameKeyword(row.id, next));
        router.refresh();
      } catch (e) {
        setError(e instanceof Error ? e.message : "Failed to rename keyword.");
      }
    });
  }
    ...
    <>
      <div className={adminStyles.filterRow}>
        ...
        <ColumnPicker
          ...
          onChange={(cols) => navigate({ cols } as Partial<KeywordsFilters>)}
          onReset={() => navigate({ cols: null } as Partial<KeywordsFilters>)}
          onSaveDefault={async (cols) => {
            await saveTableColumns("keywords", cols);
            navigate({ cols: null } as Partial<KeywordsFilters>);
          }}
        ...
        <table className={adminStyles.table}>
          ...
          <tbody>
            {displayRows.length === 0 && (
              <EmptyRow colSpan={visibleColumns.length} message="No keywords matching the criteria." />
            )}
            {displayRows.map((row) => (
      ...
      <FilterHelp
        ...
        notes={
          <p style={{ marginTop: 8 }}>
            ...
            <code>keyword_metrics</code> view — filtered counts over live acts of tagging whose target is itself live,
