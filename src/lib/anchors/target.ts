...
/** The column shape shared by `tag_anchor`, `anchored_link_anchor` and `comment_quote_anchor`. */
...
export function targetToColumns(target: AnchorTarget): AnchorTargetColumns {
  return {
    ...
    targetCommentId: target.kind === "comment" ? target.id : null,
  };
}
...
export function targetFromColumns(columns: AnchorTargetColumns): AnchorTarget | null {
  ...
  if (columns.targetCommentId !== null) set.push({ kind: "comment", id: columns.targetCommentId });
