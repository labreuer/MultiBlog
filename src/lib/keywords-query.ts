...
// /keywords' querystring vocabulary (PLAN.md §20d). The admin-table kit's
// sixth `*-query.ts`, and deliberately the plainest of them: a keyword has no
...
//
// Six keys here can't be reached by a plain `ORDER BY` on `keyword`, and all
// six go through the `keyword_metrics` view (§16e, §16l):
//
...
// and would otherwise lead a descending sort (src/app/keywords/page.tsx).
//
...
const SORT_KEYS: readonly KeywordsSortKey[] = [
  "name",
  "description",
  "assignments",
  "docs",
...
export const DEFAULT_SORT: SortColumn<KeywordsSortKey>[] = [{ key: "name", dir: "asc" }];

export type KeywordsFilters = BaseFilters<KeywordsSortKey>;

function spec(prefs: TablePrefs): BaseFilterSpec<KeywordsSortKey> {
  return { sortKeys: SORT_KEYS, defaultSort: DEFAULT_SORT, prefs };
}

export function parseKeywordsFilters(searchParams: URLSearchParams, prefs: TablePrefs): KeywordsFilters {
  return parseBaseFilters(searchParams, spec(prefs));
}

export function buildKeywordsQueryString(filters: KeywordsFilters, extra: URLSearchParams, prefs: TablePrefs): string {
