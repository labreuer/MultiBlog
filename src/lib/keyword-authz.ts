...
export { canApplyKeywords, canCurateKeywords } from "@/lib/role-checks";
...
// floor (canApplyKeywords, ADMIN/EDITOR/AUTHOR/AUTHORIZED) is what keeps
...
// restated. Reusing them is the whole point: a keyword chip on a PRIVATE doc
...
// /keyword/[slug] lists, for everyone. So the three surfaces are consistent
...
export async function canUserTagTarget(userId: string, role: Role, target: AnchorTarget): Promise<boolean> {
  if (!canApplyKeywords(role)) return false;
  ...
  switch (target.kind) {
    ...
    case "post": {
      ...
      // and /keyword/[slug] would then be the surface that leaked it.
 ...
 * is attributable (`keyword_assignment.user_id`), and letting any AUTHORIZED
...
export function canUserRemoveAssignment(userId: string, role: Role, assignmentUserId: string): boolean {
  return assignmentUserId === userId || canCurateKeywords(role);
}
