...
// PLAN.md §20d — the reads behind keyword chips.
//
// **One indexed `keyword_anchor` lookup per object page**, by the arc column
...
export type KeywordChip = {
  /** The keyword, not the assignment — what the chip links to. */
  id: string;
  slug: string;
  name: string;
  /**
 ...
 *
 * Soft-deleted assignments are excluded **by hand**: `keywordAssignment`
 ...
 * `deletedAt: null` itself. Soft-deleted *keywords* need no such clause — that
 * model does join the filter — but the nested `keyword` relation here is
 * likewise out of the filter's reach, so it is stated too. Both are the kind
 * of thing that is invisible when wrong.
 */
export async function keywordsForTarget(target: AnchorTarget, viewerId?: string): Promise<KeywordChip[]> {
  const columns = targetToColumns(target);
  const anchors = await prisma.keywordAnchor.findMany({
    where: {
    ...
    select: {
      assignment: {
        select: {
          id: true,
          userId: true,
          keyword: { select: { id: true, slug: true, name: true } },
        },
      },
    },
  });
  ...
  return [...byKeyword.values()].sort((a, b) => a.name.localeCompare(b.name));
}

export type KeywordOption = { id: string; slug: string; name: string };
 ...
 */
export async function listKeywordOptions(): Promise<KeywordOption[]> {
...
export async function keywordBySlug(slug: string) {
