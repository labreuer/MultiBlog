...
export const prisma = client.$extends({
  query: {
    ...
    // deletedByUserId/deletedAt pair, and an admin table (/keywords) that
    // reaches for prismaIncludingDeleted precisely so it can offer a restore.
    //
    // `keywordAssignment` is deliberately NOT here, and filters by hand
    // instead. This extension intercepts *top-level* operations only, and an
    // assignment is read almost exclusively through a `keyword_anchor`
