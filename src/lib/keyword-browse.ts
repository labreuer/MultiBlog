...
// PLAN.md §20d — what /keyword/[slug] lists, as **per-type sections**.
//
...
//
// Each is one indexed `keyword_anchor` lookup joined to its type's table, so
...
// `keyword_metrics`** (§20d). The view counts everything live; these count what
...
export type KeywordHit = { id: string; slug: string; title: string; href: string };

export type KeywordBrowse = {
  docs: KeywordHit[];
  posts: KeywordHit[];
  files: KeywordHit[];
...
/** The shared shape of "which live assignments of this keyword point at me". */
function taggedWith(keywordId: string) {
  return {
    some: {
      assignment: { keywordId, deletedAt: null, keyword: { deletedAt: null } },
    },
  };
}

export async function browseKeyword(keywordId: string, userId: string | null, role: Role | null): Promise<KeywordBrowse> {
  const [docs, posts, files] = await Promise.all([
    listDocs(keywordId, userId, role),
    listPosts(keywordId),
    listFiles(keywordId, userId, role),
  ]);
  return {
    docs,
    posts,
...
async function listDocs(keywordId: string, userId: string | null, role: Role | null): Promise<KeywordHit[]> {
  ...
  const rows = await prisma.doc.findMany({
    where: { deletedByUserId: null, OR: or, keywordAnchors: taggedWith(keywordId) },
...
async function listPosts(keywordId: string): Promise<KeywordHit[]> {
  const rows = await prisma.post.findMany({
    where: { ...publishedPostWhere(), keywordAnchors: taggedWith(keywordId) },
    select: { id: true, slug: true, title: true },
...
async function listFiles(keywordId: string, userId: string | null, role: Role | null): Promise<KeywordHit[]> {
  if (!userId || !role) return [];
  const or = [];
