...
// PLAN.md §20d — mutations on the keyword vocabulary and on individual acts of
...
// and takes its gate from the matching half of src/lib/keyword-authz.ts.
//
...
// keyword_anchor_selector_columns_check makes that structural rather than
// merely true today.
...
async function requireCurator() {
  ...
  if (!canCurateKeywords(session.user.role)) {
    throw new Error("Only an admin or editor can manage keyword terms.");
  }
  return session;
}
...
function toTarget(kind: string, id: string): AnchorTarget {
  ...
  if (!parsed) {
    throw new Error(`"${kind}" is not something a keyword can be attached to.`);
  }
 ...
 *
 * `/keyword/[slug]` deliberately gets no revalidation: it renders dynamic,
 ...
 * actually opens it.
 *
 * Not props on the page, deliberately. The vocabulary is the whole keyword
 ...
 * keyword Epistemology" means the same thing whether or not somebody typed it
 ...
 * (case-insensitively, per `keyword_name_lower_key`), not the slug.
 */
export async function createKeyword(nameInput: string, descriptionInput?: string): Promise<{ id: string; slug: string; name: string }> {
  const session = await requireTagger();
  if (!canApplyKeywords(session.user.role)) {
    throw new Error("Your account doesn't have permission to apply keywords.");
  }

  const name = nameInput.trim().replace(/\s+/g, " ").slice(0, MAX_NAME_LENGTH);
  if (!name) {
    throw new Error("A keyword needs a name.");
  }
  ...
  if (await keywordNameInUse(name)) {
    throw new Error(`"${name}" already exists as a deleted keyword — an admin or editor can restore it.`);
  }

  const description = descriptionInput?.trim().slice(0, MAX_DESCRIPTION_LENGTH) || null;
  const keyword = await prisma.keyword.create({
    data: { slug: await uniqueKeywordSlug(name), name, description, createdById: session.user.id },
    select: { id: true, slug: true, name: true },
  });
...
/**
 * Applies `keywordId` to one whole object, as one act of tagging.
 *
 * **Dedup is app-level find-first** (§20c): same keyword, same object, same
 * user means no second assignment, and re-tagging is a no-op rather than an
 * error. The DB-enforced version needs `keyword_id` denormalised onto the
 ...
 * losing race just leaves a duplicate chip, which `keywordsForTarget`
 * collapses anyway.
 *
 ...
 * second concept.
 */
export async function tagObject(keywordId: string, targetKind: string, targetId: string): Promise<void> {
  const session = await requireTagger();
  const target = toTarget(targetKind, targetId);
  ...
  const keyword = await prisma.keyword.findUnique({ where: { id: keywordId }, select: { id: true } });
  if (!keyword) {
  ...
  const already = await prisma.keywordAnchor.findFirst({
    where: {
      ...columns,
      assignment: { keywordId, userId: session.user.id, deletedAt: null },
    },
    select: { id: true },
  });
  if (already) return;
  ...
  await prisma.$transaction(async (tx) => {
    const assignment = await tx.keywordAssignment.create({
      data: { keywordId, userId: session.user.id },
      select: { id: true },
    });
    await tx.keywordAnchor.create({
...
export async function untagObject(assignmentId: string): Promise<void> {
  ...
  const assignment = await prisma.keywordAssignment.findFirst({
 ...
 * re-slugging here would break every existing `/keyword/…` link with nothing
 * to redirect them. Changing the URL is therefore a deliberate separate act
 * (`updateKeywordSlug`) rather than a side effect of fixing a typo.
 */
export async function renameKeyword(keywordId: string, nameInput: string, descriptionInput?: string): Promise<void> {
  ...
  if (await keywordNameInUse(name, keywordId)) {
    throw new Error(`Another keyword is already called "${name}".`);
  }

  await prismaIncludingDeleted.keyword.update({
    where: { id: keywordId },
    data: {
      name,
 ...
 *
 * Separated from `renameKeyword` precisely so that cost is chosen rather than
 * incurred. The first time a keyword URL is shared somewhere durable is the
 * trigger condition for building `keyword_slug_history` (§20i), at which point
 * this becomes `changeDocSlug`'s twin and the warning goes away.
 */
export async function updateKeywordSlug(keywordId: string, slugInput: string): Promise<{ slug: string }> {
  await requireCurator();
  const slug = await uniqueKeywordSlug(slugInput, keywordId);
  await prismaIncludingDeleted.keyword.update({ where: { id: keywordId }, data: { slug } });
  revalidatePath("/keywords");
  return { slug };
}

async function setKeywordDeleted(keywordId: string, deleted: boolean): Promise<void> {
  ...
  const keyword = await prismaIncludingDeleted.keyword.findUnique({
    where: { id: keywordId },
    select: { id: true },
  });
  if (!keyword) {
 ...
 * *reader* filters on its own `deleted_at` — `keywordsForTarget`'s nested
 * `keyword: { deletedAt: null }`, and the `$extends` filter everywhere the
 * model is read top-level — so a deleted term draws no chips and answers no
 * `/keyword/[slug]` while its history stays intact. That is the whole
 * difference between a soft delete and a hard one.
 *
 * Note `keyword_metrics` does *not* filter on it, and needn't: the view is
 * keyed on keyword_id and joined per row, so a deleted term's usage numbers
 * simply travel with the row `/keywords` is already only showing under "show
 * deleted".
 */
export async function deleteKeyword(keywordId: string): Promise<void> {
  await setKeywordDeleted(keywordId, true);
}

export async function restoreKeyword(keywordId: string): Promise<void> {
  await setKeywordDeleted(keywordId, false);
}

// Per-row rather than one transaction — see bulkDeletePosts for the rationale.
export async function bulkDeleteKeywords(keywordIds: string[]): Promise<BulkResult> {
  return settleBulk(keywordIds, (id) => setKeywordDeleted(id, true));
}

export async function bulkRestoreKeywords(keywordIds: string[]): Promise<BulkResult> {
  return settleBulk(keywordIds, (id) => setKeywordDeleted(id, false));
}
