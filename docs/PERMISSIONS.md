...
| `canApplyKeywords` | ✅ | ✅ | ✅ | ✅ | ❌ | `role-checks.ts` |
| `canCurateKeywords` | ✅ | ✅ | ❌ | ❌ | ❌ | `role-checks.ts` |
...
| `/keywords` (the admin table) | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ |

**Chips are as private as the thing they are on, structurally.** `KeywordChips` is rendered
...
shows come from those filtered queries and **never** from the `keyword_metrics` view, which
...
`/keyword/[slug]` lists, for everyone. So all three surfaces take the same role floor, and
...
curation and is a one-line change in `role-checks.ts` plus a branch in `createKeyword`. Worth
making the moment the vocabulary shows drift rather than growth; not worth pre-empting.

**`/keywords` sets the same bar as every other admin table** (`canManageDocs`, AUTHOR and up)
rather than matching `canApplyKeywords`. An AUTHORIZED user who may create and apply terms
still cannot open the table — they reach the vocabulary through the tagger's picker and
through `/keyword/[slug]`. Inventing a seventh visibility tier for one listing would be a UI
...
term's own `deleted_at`, so a deleted term draws no chips and answers no `/keyword/[slug]`,
...
| Keyword role floors (`canApplyKeywords`, `canCurateKeywords`) | `src/lib/role-checks.ts` |
| Who may tag which object; who may retract an assignment | `src/lib/keyword-authz.ts` |
| Keyword mutations (create, tag, untag, rename, slug, delete) | `src/app/actions/keywords.ts` |
| `/keyword/[slug]`'s three per-type predicates | `src/lib/keyword-browse.ts` |
| `/keywords` row scoping (there is none) + the curate gate | `src/app/keywords/page.tsx` |
...
`e2e/keywords.spec.ts` pins the keyword table's load-bearing rows — the signed-out reader
