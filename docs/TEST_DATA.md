...
- **`scripts/test-keyword.ts`** — create/tag/untag/list/delete throwaway keywords (PLAN.md
...
`test-keyword.ts` needs **two** guards, and the second one is the non-obvious half. The first
is the usual one: the term's creator must be an `@example.com` account. The second is that
deleting a keyword *cascades its assignments*, so a throwaway term a **real** account has
